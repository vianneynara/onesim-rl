...
def load_telegram_config(config_path: str = "telegram_config.json") -> dict:
    ...
    if not os.path.exists(config_path):
        log.info("[TELEGRAM] Config file '%s' not found. Telegram notifications disabled.", config_path)
        return None
    try:
        with open(config_path, "r") as f:
            cfg = json.load(f)
        if "bot_token" not in cfg or "chat_id" not in cfg:
            log.warning("[TELEGRAM] Config missing 'bot_token' or 'chat_id'. Telegram notifications disabled.")
            return None
        return cfg
    except Exception as e:
        log.warning("[TELEGRAM] Failed to load config: %s. Telegram notifications disabled.", e)
        return None
...
def send_telegram(cfg: dict, message: str) -> bool:
    ...
    try:
        ...
        log.warning("[TELEGRAM] Failed to send message: %s", e)
        return False


# ------------------------------------------------------------------------------------------------------------------- #
# TELEGRAM /stop AND /status LISTENER
# ------------------------------------------------------------------------------------------------------------------- #
            ...
            try:
                ...
                for update in data.get("result", []):
                    ...
                    if text == "/stop":
                        with stop_lock:
                            stop_requested = True
                        log.info("[TELEGRAM] /stop received — will stop after current config finishes.")
                        send_telegram(cfg,
                            "🛑 <b>/stop received</b>\n"
...
def beep_done():
    ...
    try:
        ...
        log.warning("[BEEP] Could not play sound: %s", e)
...
def run_simulation(
    ...
    send_telegram(
        ...
        f"Episodes: {start_ep_override}–{runs}\n"
    ...
    with status_lock:
        ...
        current_status["bp"] = bp or ""
        current_status["run_id"] = result_id_dir
        current_status["total_ep"] = runs
        current_status["start_ep"] = effective_start_ep
        current_status["current_ep"] = effective_start_ep
        current_status["config_start_time"] = start_time

    for ep in range(effective_start_ep, runs + 1):
        with status_lock:
    ...
    send_telegram(
        ...
        f"Algorithm: <code>{alg}</code> | Policy: <code>{bp}</code>\n"
        f"Run ID: <code>{result_id_dir}</code>\n"
        f"Episodes: {effective_start_ep}–{runs} (✅ {succeeds} / ❌ {failed})\n"
...
if __name__ == "__main__":
    ...
    if tg_cfg:
        ...
        log.info("[TELEGRAM] Listener started. Commands: /stop (stop after current config) | /status (show progress)")
    else:
    ...
    if args.all:
        ...
        log.info("Running %s configurations.", total_batches)
        for idx, config in enumerate(LIST_OF_CONFIGS, start=1):
            with stop_lock:
                should_stop = stop_requested
            if should_stop:
                log.info("[INFO] Stop requested — skipping remaining configs (stopped before config %s/%s).", idx, total_batches)
                send_telegram(tg_cfg,
            ...
            start_ep_override = config.get("start_ep", 1)
            ...
            success = run_simulation(
                ...
                start_ep_override=start_ep_override,
            )
