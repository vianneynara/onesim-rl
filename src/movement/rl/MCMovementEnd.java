...
//	private final int episodeNumber;
	...
	private static final Map<StateActionPair, Double> episodeSteps = new HashMap<>();
...
//		System.out.println("is it loading this");
	...
	public static void reset() {
		...
		if (s.contains(LEARNING_SEED_S) && s.getInt(LEARNING_SEED_S) != 0) {
			System.out.printf("[%s] Using %s.%s of: %s %n", MCMovementEnd.class.getCanonicalName(), MONTECALRO_NS, LEARNING_SEED_S, s.getInt(LEARNING_SEED_S));
			learningRNG = new Random(s.getInt(LEARNING_SEED_S));
		} else if (s.contains(LEARNING_SEED_S) && s.getInt(LEARNING_SEED_S) == 0) {
			System.out.printf("[%s] Using a random learning RNG seed.", MCMovementEnd.class.getCanonicalName());
			learningRNG = new Random();
		} else {
			// Inherit the seed from MovementModel
			System.out.printf("[%s] Using a random learning RNG seed inherited from MovementModel.", MCMovementEnd.class.getCanonicalName());
...
//		setQ(state, action, newQ);
		...
		episodeSteps.put(new StateActionPair(state, action), reward);
	}
	...
	private void updateEpisode() {
		for (Map.Entry<StateActionPair, Double> entry : episodeSteps.entrySet()) {
			int state = (int) entry.getKey().getStateId();
			int action = entry.getKey().getAction();
	...
	/**
	 * Getting Q-value of state-action pair (Q(s,a)). Defaults to {@link MCMovementEnd#initialQValue}.
	 *
	 */
...
//		if (SimClock.getTime() % 1000 == 1) System.out.println("CURRENT POSITION: " + currentPosition);
	...
	/**
	 * Records a length value to {@link MCMovementEnd#trajectoryFrequencies}.
	 */
	...
	/**
	 * Returns the true amount of detections, derived from adding all occurrences in {@link MCMovementEnd#objectiveFound}.
	 *
	 */
	@Override
