 ...
 *
 */
public class MCMovement extends MovementModel implements TrajectoryFrequencyReporting, QTableReporting, RewardReporting, SearchingAgentReporting, SearchingOccurrencesReporting, EpisodicPersistable {
	// [ REPORTING VARIABLES ]
...
//	private final int episodeNumber;
	...
	public static final String MONTECALRO_NS = "MCMovement";
	 ...
	 *
	 * StateActionPair, Prev Q-Value, New Q-Value
	 */
	private final Map<StateActionPair, Tuple<Double, Double>> qTable;
	...
	static {
		DTNSim.registerForReset(MCMovement.class.getCanonicalName());
		reset();
	}

	public MCMovement(Settings settings) {
		super(settings);
...
//		System.out.println("is it loading this");
	 ...
	 */
	public MCMovement(MCMovement proto) {
		super(proto);
		...
		this.stateActionCount = proto.stateActionCount;
	...
	public static void reset() {
		...
		if (s.contains(LEARNING_SEED_S) && s.getInt(LEARNING_SEED_S) != 0) {
			System.out.printf("[%s] Using %s.%s of: %s %n", MCMovement.class.getCanonicalName(), MONTECALRO_NS, LEARNING_SEED_S, s.getInt(LEARNING_SEED_S));
			learningRNG = new Random(s.getInt(LEARNING_SEED_S));
		} else if (s.contains(LEARNING_SEED_S) && s.getInt(LEARNING_SEED_S) == 0) {
			System.out.printf("[%s] Using a random learning RNG seed.", MCMovement.class.getCanonicalName());
			learningRNG = new Random();
		} else {
			// Inherit the seed from MovementModel
			System.out.printf("[%s] Using a random learning RNG seed inherited from MovementModel.", MCMovement.class.getCanonicalName());
	...
	protected int selectAction(int state, Set<Integer> availableActions) {
		...
		for (Integer action : availableActions) {
			qValues.put(action, getPrevQ(state, action));
		}
	 ...
	 *
	 */
	// Ini harus di ubah menjadi Monte Carlo
	protected void update(int state, int action, double reward, int nextState, Set<Integer> availableNextActions) {
		...
		this.G = lambda * G + reward;

		StateActionPair tempKey = new StateActionPair(state, action);

		// First Occurrences
		if (!this.stateActionCount.containsKey(tempKey)) {
			this.stateActionCount.put(tempKey, 1);
		} else {
			int count = this.stateActionCount.get(tempKey);
			this.stateActionCount.put(tempKey, ++count);
		}

		double oldQ = getNewQ(state, action);
		int count = this.stateActionCount.get(tempKey);
		...
		// First Visit
		if (firstVisit) {
			if (this.stateActionCount.get(tempKey) == 1) {
	...
	/**
	 * Getting Q-value of state-action pair (Q(s,a)). Defaults to {@link MCMovement#initialQValue}.
	 *
	 */
	private double getPrevQ(int state, int action) {
		Tuple<Double, Double> temp = qTable.getOrDefault(StateActionPair.of(state, action), new Tuple(initialQValue, initialQValue));
		return temp.getKey();
	}
	 ...
	 *
	 */
	private void setPrevQ(int state, int action, double value) {
		qTable.put(StateActionPair.of(state, action), new Tuple<>(value, getNewQ(state, action)));
	}

	private double getMaxPrevQ(int state, Set<Integer> availableActions) {
		double maxQ = Double.NEGATIVE_INFINITY;

		// Getting the maximum Q-Value by comparing each action's q-value for the state
		for (Integer action : availableActions) {
			maxQ = Math.max(maxQ, getPrevQ(state, action));
		}
	 ...
	 *
	 */
	private double getNewQ(int state, int action) {
		Tuple<Double, Double> temp = qTable.getOrDefault(StateActionPair.of(state, action), new Tuple(initialQValue, initialQValue));
	 ...
	 *
	 */
	private void setNewQ(int state, int action, double value) {
		qTable.put(StateActionPair.of(state, action), new Tuple<>(getPrevQ(state, action), value));
	}

	private double getMaxNewQ(int state, Set<Integer> availableActions) {
		double maxQ = Double.NEGATIVE_INFINITY;

		// Getting the maximum Q-Value by comparing each action's q-value for the state
		for (Integer action : availableActions) {
			maxQ = Math.max(maxQ, getNewQ(state, action));
		}
	 ...
	 *
	 */
	// flow update harus diubah sesuai dengan Monte Carlo
	@Override
	public Path getPath() {
...
//				s_t, a_t, reward, s_tp1, availableActions);
			update(s_t, a_t, reward, s_tp1, availableActions);		// THISSSS FUCKKKKK 1
		}
...
//		if (SimClock.getTime() % 1000 == 1) System.out.println("CURRENT POSITION: " + currentPosition);
	...
	/**
	 * Records a length value to {@link MCMovement#trajectoryFrequencies}.
	 */
	...
	public Map<StateActionPair, Double> getQTable() {
		Map<StateActionPair, Double> newQTable = new HashMap<>();
		for (Map.Entry<StateActionPair, Tuple<Double, Double>> entry : qTable.entrySet()) {
			newQTable.put(entry.getKey(), entry.getValue().getValue());
		}
		return newQTable;
	}
	...
	/**
	 * Returns the true amount of detections, derived from adding all occurrences in {@link MCMovement#objectiveFound}.
	 *
	 */
	@Override
	...
	public void saveTo(EpisodicPersistenceData epd) {
		System.out.printf("[%s] Saving persistence data...%n", MCMovement.class.getCanonicalName());
		...
		for (var entr : qTable.entrySet()) {
			...
			epd.qTable.put(key, entr.getValue().getValue());
		}
	...
	public void loadFrom(EpisodicPersistenceData epd) {
		System.out.printf("[%s] Loading persistence data...%n", MCMovement.class.getCanonicalName());
...
//		System.out.println("Loading EPD.currentPosition: " + epd.currentPosition);
		...
		if (epd.qTable != null) {
			for (var entry : epd.qTable.entrySet()) {
				qTable.put(
					StateActionPair.fromJsonKey(entry.getKey()),
					new Tuple<>(entry.getValue(), entry.getValue())
				);
			}
		}
